...
const GiftCardMyntra = () => {
  ...
  useEffect(() => {
    const fetchVouchers = async () => {
      try {
        const response = await fetch(`${BACKEND_URL}/api/products/category/gift-cards?brand=Myntra&limit=50`)
        ...
        console.error('Error fetching Myntra vouchers:', error)
        setVouchers(defaultVouchers)
      } finally {
        setLoading(false)
      }
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 max-w-4xl mx-auto">
            {vouchers.map((voucher) => {
              ...
              const image = voucher.images?.[0] || '/products/myntra.avif'
