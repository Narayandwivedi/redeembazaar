...
const GiftCardFlipkart = () => {
  ...
  useEffect(() => {
    const fetchVouchers = async () => {
      try {
        const response = await fetch(`${BACKEND_URL}/api/products/category/gift-cards?brand=Flipkart&limit=50`)
        ...
        console.error('Error fetching Flipkart vouchers:', error)
        setVouchers(defaultVouchers)
      } finally {
        setLoading(false)
      }
  ...
  useSEO({
    ...
    canonicalUrl: 'https://redeemkart.in/gift-cards/flipkart',
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 sm:gap-6 lg:gap-8 max-w-5xl mx-auto">
            {vouchers.map((voucher) => {
              ...
              const image = voucher.images?.[0] || '/products/flipkart.avif'
                  ...
                  <div className="p-4 flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mb-3">
                        <span className="text-[10px] sm:text-xs tracking-wider font-bold text-emerald-600 uppercase">Flipkart Code</span>
                      </div>
                    </div>
