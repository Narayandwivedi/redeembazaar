                  ...
                  <tr key={card._id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2 py-1 rounded text-xs font-bold uppercase ${
                        ...
                        ['sold', 'sold_out'].includes(card.status) ? 'bg-blue-100 text-blue-700' :
                      ...
                      }`}>{['sold', 'sold_out'].includes(card.status) ? 'sold out' : card.status}</span>
                      {['sold', 'sold_out'].includes(card.status) && card.paidOn && (
                        <span className="inline-flex items-center px-2 py-1 rounded text-xs font-bold uppercase bg-green-100 text-green-700 ml-1">Paid</span>
                      )}
                      {['sold', 'sold_out'].includes(card.status) && card.soldTo && (
                        <span className="block text-[10px] text-gray-500 mt-1">Buyer: {card.soldTo.fullName} ({card.soldTo.email})</span>
                      )}
                      {(card.paidOn || ['sold', 'sold_out', 'paid'].includes(card.status)) && card.paidOn && (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-sm p-6">
            ...
            <p className="text-xs text-gray-500 mt-2">
              Marks the seller as paid. If the card is not Sold Out yet, it will be marked as <span className="font-semibold">Sold Out</span> and removed from the storefront. A Sold Out card that is paid will show in both the Sold and Paid filters.
            </p>
            <div className="flex justify-end gap-2 mt-6">
              <button
